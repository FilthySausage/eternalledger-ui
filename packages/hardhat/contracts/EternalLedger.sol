...
contract EternalLedger is ERC721, Ownable {
    ...
    constructor() ERC721("Eternal Ledger", "ETERNAL") Ownable(msg.sender) {
        ...
        deploymentTs = uint96(block.timestamp);
    }
    ...
    function recordDeath(string memory nric, string memory metadataCID) external onlyRegistrar {
        require(bytes(nric).length > 0, "NRIC cannot be empty");
        ...
        records[tokenId] = Record({ metadataCID: metadataCID, timestamp: block.timestamp });
    ...
    function getDeathStatistics()
        external
        view
        returns (uint256 totalDeaths, uint256 oldestTokenId, uint256 newestTokenId, uint256 contractCreationTime)
    {
        return (
            totalSupply,
            totalSupply > 0 ? 1 : 0,
